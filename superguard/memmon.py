...
class Memmon:
    ...
    def runforever(self, test=False):
        while 1:
            ...
            if self.programs:
                ...
                status.append(
                    "Checking programs %s"
                    % ", ".join(["%s=%s" % (k, self.programs[k]) for k in keys])
                )
            ...
            if self.groups:
                ...
                status.append(
                    "Checking groups %s"
                    % ", ".join(["%s=%s" % (k, self.groups[k]) for k in keys])
                )
    ...
    def restart(self, name, rss):
        ...
        if self.email and uptime <= self.email_uptime_limit:
            ...
            subject = self.format_subject("process %s restarted" % name)
            self.mail(self.email, subject, msg)
...
seconds_size = SuffixMultiplier({"s": 1, "m": 60, "h": 60 * 60, "d": 60 * 60 * 24})
