...
import time
import unittest
from superguard.compat import StringIO
from supervisor.process import ProcessStates
from superguard.tests.dummy import DummyResponse
from superguard.tests.dummy import DummyRPCServer
from superguard.tests.dummy import DummySupervisorRPCNamespace
...
class HTTPOkTests(unittest.TestCase):
    def _getTargetClass(self):
        from superguard.httpok import HTTPOk
    ...
    def test_listProcesses_w_RUNNING_programs_default_state(self):
        ...
        self.assertEqual(specs[0], DummySupervisorRPCNamespace.all_process_info[0])
    ...
    def test_listProcesses_w_nonRUNNING_programs_default_state(self):
        ...
        specs = list(prog.listProcesses())
        self.assertEqual(len(specs), 1)
        self.assertEqual(specs[0], DummySupervisorRPCNamespace.all_process_info[1])
    ...
    def test_runforever_acts_if_status_is_unexpected(self):
        ...
        self.assertTrue(
            "Subject: httpok: http://foo/bar: " "bad status returned" in lines
        )
        self.assertTrue(
            "status contacting http://foo/bar: " "500 Internal Server Error" in lines
        )
    ...
    def test_runforever_acts_if_inbody_isnt_present(self):
        ...
        lines = prog.stderr.getvalue().split("\n")
        self.assertTrue(
            "Subject: httpok: http://foo/bar: " "bad body returned" in lines
        )
    ...
    def test_runforever_eager_error_on_request_some(self):
        ...
        self.assertEqual(lines[4], "baz:baz_01 not in RUNNING state, NOT restarting")
        self.assertEqual(
            lines[5],
    ...
    def test_runforever_eager_error_on_process_stop(self):
        ...
        self.assertEqual(
            lines[2], "Failed to stop process foo:FAILED: <Fault 30: 'FAILED'>"
        )
    ...
    def test_runforever_eager_error_on_process_start(self):
        ...
        prog.runforever(test=True)
        lines = prog.stderr.getvalue().split("\n")
        self.assertEqual(lines[0], "Restarting selected processes ['SPAWN_ERROR']")
        self.assertEqual(lines[1], "foo:SPAWN_ERROR is in RUNNING state, restarting")
        self.assertEqual(
            lines[2],
    ...
    def test_runforever_eager_gcore(self):
        ...
        any = None
        prog = self._makeOnePopulated(
            programs, any, exc=True, gcore="true", coredir="/tmp"
        )
        prog.stdin.write("eventname:TICK len:0\n")
        prog.stdin.seek(0)
        prog.runforever(test=True)
        ...
        self.assertEqual(lines[7], "baz:baz_01 not in RUNNING state, NOT restarting")
        self.assertEqual(
            lines[8],
            "Programs not restarted because they did not exist: ['notexisting']",
        )
    ...
    def test_runforever_not_eager_none_running(self):
        ...
        any = None
        prog = self._makeOnePopulated(
            programs, any, exc=True, gcore="true", coredir="/tmp", eager=False
        )
        prog.stdin.write("eventname:TICK len:0\n")
        prog.stdin.seek(0)
        prog.runforever(test=True)
    ...
    def test_runforever_not_eager_running(self):
        ...
        prog.runforever(test=True)
        lines = [x for x in prog.stderr.getvalue().split("\n") if x]
        self.assertEqual(lines[0], ("Restarting selected processes ['foo', 'bar']"))
        self.assertEqual(lines[1], "foo is in RUNNING state, restarting")
        self.assertEqual(lines[2], "foo restarted")
        self.assertEqual(lines[3], "bar not in RUNNING state, NOT restarting")
        mailed = prog.mailed.split("\n")
    ...
    def test_runforever_connrefused_error(self):
        ...
        error.errno = 111
        prog = self._makeOnePopulated(
            programs, any, exc=[error for x in range(100)], eager=False
        )
        prog.stdin.write("eventname:TICK len:0\n")
        prog.stdin.seek(0)
        prog.runforever(test=True)
    ...
    def test_bug_110(self):
        ...
        prog.runforever(test=True)
        lines = [x for x in prog.stderr.getvalue().split("\n") if x]
        self.assertEqual(lines[0], ("Restarting selected processes ['foo']"))
        self.assertEqual(lines[1], "foo is in RUNNING state, restarting")
        self.assertEqual(lines[2], "foo restarted")
