...
import sys
import time
from superguard.compat import urlparse
...
from superguard import timeoutconn
...
class HTTPOk:
    ...
    def listProcesses(self, state=None):
        return [
            x
            for x in self.rpc.supervisor.getAllProcessInfo()
            if x["name"] in self.programs and (state is None or x["state"] == state)
        ]
            ...
            if self.eager or len(specs) > 0:
                ...
                try:
                    ...
                    for will_retry in range(
                        (self.timeout - 1) // (self.retry_time or 1), -1, -1
                    ):
                        try:
                ...
                if status not in self.statuses:
                    subject = self.format_subject("%s: bad status returned" % self.url)
                    self.act(subject, msg)
                elif self.inbody and self.inbody not in body:
                    subject = self.format_subject("%s: bad body returned" % self.url)
                    self.act(subject, msg)
    ...
    def act(self, subject, msg):
        ...
        if waiting:
            write("Programs not restarted because they did not exist: %s" % waiting)
    ...
    def restart(self, spec, write):
        ...
        if spec["state"] is ProcessStates.RUNNING:
            ...
            try:
                ...
                write("Failed to stop process %s: %s" % (namespec, e))

            try:
                self.rpc.supervisor.startProcess(namespec)
            except xmlrpclib.Fault as e:
                write("Failed to start process %s: %s" % (namespec, e))
            else:
...
def main(argv=sys.argv):
    ...
    try:
        ...
        if e.args[0] != "SUPERVISOR_SERVER_URL":
            raise
        sys.stderr.write("httpok must be run as a supervisor event " "listener\n")
        sys.stderr.flush()
        return
